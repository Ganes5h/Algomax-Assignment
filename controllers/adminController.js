...
exports.loginAdmin = async (req, res) => {
  try {
    ...
    const [admin] = await query(`SELECT * FROM admins WHERE email = ?`, [
      email,
    ]);
    if (!admin) {
    ...
    res.status(200).json({ message: "Login successful", token, adminId });
  } catch (error) {
...
exports.verifyTenantByAdmin = async (req, res) => {
  try {
    ...
    if (!tenantId || !adminId || !["approved", "rejected"].includes(status)) {
      return res.status(400).json({
        message:
          "Tenant ID, Admin ID, and valid status (approved/rejected) are required",
      });
    }
    ...
    if (tenant.admin_verification_status !== "pending") {
      return res
        .status(400)
        .json({ message: "Tenant is already verified or rejected" });
    }
    ...
    const [updatedTenant] = await query(`SELECT * FROM tenants WHERE id = ?`, [
      tenantId,
    ]);
...
exports.getDocumentPathsByTenantId = async (req, res) => {
  ...
  try {
    ...
    if (documentPaths.length === 0) {
      return res
        .status(404)
        .json({ message: "No documents found for the specified tenant" });
    }
